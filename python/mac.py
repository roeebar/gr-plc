...
class mac(gr.basic_block, Machine):
        ...
        if gr.pmt.is_pair(msg):
            ...
            if gr.pmt.is_symbol(car) and gr.pmt.is_dict(cdr):
                ...
                if msg_id == "MAC-TXMSDU":
                    ...
                    self.logger.debug("state = " + str(self.state) + ", received payload from APP, size = " + str(len(payload)) + ", frames in buffer = " + str(self.tx_frames_in_queue))
    ...
    def phy_in_handler(self, msg):
        if gr.pmt.is_pair(msg):
            ...
            if gr.pmt.is_symbol(car) and gr.pmt.is_dict(cdr):
                msg_id = gr.pmt.to_python(car)
                dict = gr.pmt.to_python(cdr)
                self.logger.debug("state = " + str(self.state) + ", received " + msg_id + " from PHY")
                ...
                if msg_id == "PHY-RXSTART":
                    ...
                    self.logger.debug("state = " + str(self.state) + ", received MPDU (" + dt + ") from PHY")
                    if dt == "SOF":
    ...
    def transmit_sack(self):
        ...
        dict = gr.pmt.make_dict()
        dict = gr.pmt.dict_add(dict, gr.pmt.to_pmt("frame_control"), mpdu_fc_pmt)
        self.logger.debug("state = " + str(self.state) + ", sending MPDU (SACK) to PHY")
        self.message_port_pub(gr.pmt.to_pmt("phy out"), gr.pmt.cons(gr.pmt.to_pmt("PHY-TXSTART"), dict))
    ...
    def send_set_tx_tone_map(self):
        ...
        self.logger.debug("state = " + str(self.state) + ", sending PHY-TXCONFIG")
    ...
    def send_init_phy(self):
        ...
        self.logger.debug("state = " + str(self.state) + ", sending PHY-RXINIT")
    ...
    def submit_mac_frame(self, frame):
        ...
        self.logger.debug("state = " + str(self.state) + ", added MAC frame to tranmission queue, frame size = " + str(len(frame)))
